    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            foreach ($variants as $index => $variant) {
                if (sizeof($this->input('options', [])) && !sizeof($variant['option_values'] ?? [])) {
                    $validator->errors()->add("variants.{$index}.option_values", "Each variant must have option values defined.");
                    continue;
                }
                ...
                if (count($this->input('options', [])) === 0) continue;
